...
import com.veadan.folib.yaml.configuration.repository.remote.PubRemoteRepositoryConfiguration;
import com.veadan.folib.yaml.configuration.repository.remote.PubRemoteRepositoryConfigurationDto;
...
public class PubRepositoryFeatures implements RepositoryFeatures {
    ...
    private void fetchRemoteSearchResult(String storageId,
        ...
        try {
            logger.debug("Search PUB packages for [{}].", remoteRepositoryUrl);
            ...
            logger.error("Failed to search PUB packages [{}]", remoteRepositoryUrl, e);
            return;
        } finally {
            restClient.close();
        }
        ...
        try {
            ...
            logger.error("Failed to parse PUB packages search result for [{}]", remoteRepositoryUrl, e);
        }
    }
    ...
    public void fetchRemoteChangesFeed(String storageId,
        ...
        PubRemoteRepositoryConfigurationDto mutableConfiguration = (PubRemoteRepositoryConfigurationDto) mutableRepository.getRemoteRepository()
                .getCustomConfiguration();

        PubRemoteRepositoryConfiguration configuration = (PubRemoteRepositoryConfiguration) remoteRepository.getCustomConfiguration();
        if (configuration == null) {
            logger.warn("Remote pub configuration not found for [{}]/[{}]", storageId, repositoryId);
            return;
        }
    ...
    private int fetchRemoteChangesFeed(Repository repository,
        ...
        RemoteRepository remoteRepository = repository.getRemoteRepository();
        PubRemoteRepositoryConfiguration repositoryConfiguration = (PubRemoteRepositoryConfiguration) remoteRepository.getCustomConfiguration();
        ...
        try (InputStream is = request.invoke(InputStream.class)) {
            ...
            Assert.isTrue(jp.nextToken() == JsonToken.START_ARRAY, "pub changes feed `results` should be array.");
            ...
            while (jp.nextToken() != null) {
                ...
                try {
                    ...
                    logger.error("Failed to parse PUB feed [{}/{}]",
    ...
    public PackageFeed fetchRemotePackageFeed(String storageId,
        ...
        try {
            ...
            logger.debug("Downloading PUB remote package feed for [{}].", url);
            ...
            logger.debug("Downloaded PUB remote package feed for [{}] take time [{}] ms.", url, System.currentTimeMillis() - startTime);
        } catch (Exception e) {
            logger.error("Failed to fetch PUB remote package feed [{}]", url, e);
            return packageFeed;
        } finally {
    ...
    public String fetchRemoteBinaryFeed(String storageId,
        ...
        try {
            ...
            service = service.path(packageId);
            url = service.getUri().toString();
            logger.debug("Downloading PUB remote binary feed for [{}].", url);
            response = service.request(MediaType.APPLICATION_JSON).get();
            if (response.getStatus() == HttpStatus.SC_OK) {
                String readString = response.readEntity(String.class);
            ...
            logger.debug("Downloaded PUB remote binary feed for [{}] take time [{}] ms.", url, System.currentTimeMillis() - startTime);
        } catch (Exception e) {
            logger.error("Failed to fetch PUB remote binary feed [{}]", url, e);
            return binaryFeed;
        } finally {
            if (Objects.nonNull(response)) {
                response.close();
    ...
    public class SearchPackagesEventListener {
        ...
        public void handle(RemoteRepositorySearchEvent event) {
            ...
            logger.debug("PUB remote repository [{}] local cached package count is [{}]",
    ...
    public PackageFeed handleViewPackage(String storageId, String repositoryId, String packageId) {
        ...
        if (Objects.nonNull(packageFeed)) {
            ...
            if (Objects.nonNull(versions) && MapUtils.isNotEmpty(versions.getAdditionalProperties())) {
                long startTime = System.currentTimeMillis();
                PubArtifactCoordinates npmArtifactCoordinates = null;
                URI uri = null;
                for (Map.Entry<String, PackageVersion> versionEntry : versions.getAdditionalProperties().entrySet()) {
                    ...
                    if (Objects.nonNull(dist) && StringUtils.isNotBlank(dist.getTarball())) {
                        npmArtifactCoordinates = PubArtifactCoordinates.of(versionEntry.getValue().getName(), versionEntry.getValue().getVersion(), packageSuffix);
