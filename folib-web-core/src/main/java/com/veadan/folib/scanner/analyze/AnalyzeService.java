package com.veadan.folib.scanner.analyze;
...
public class AnalyzeService {
     ...
     * 创建优先级队列，使用自然排序
     */
    private final OptimizedDynamicPriorityBlockingQueue<AnalyzeSbomTask> queue = new OptimizedDynamicPriorityBlockingQueue<>(Comparator.naturalOrder());
     ...
     * @param task 扫描任务
     */
    public void addTask(AnalyzeSbomTask task) {
    ...
    public void updateTaskPriority(String taskId, int newPriority) {
        AnalyzeSbomTask taskToAdjust = queue.getElementById(taskId);
        if (taskToAdjust != null) {
            ...
            queue.adjustPriority(taskToAdjust, Comparator.comparingInt(AnalyzeSbomTask::getPriority).reversed());
        }
     ...
     * 获取下一个任务
     *
     * @return AnalyzeSbomTask
     */
    public AnalyzeSbomTask getNextTask() {
        AnalyzeSbomTask task = null;
        try {
