...
public class RawSyncArtifactProvider implements SyncArtifactProvider {
    ...
    public void register() {
        syncArtifactProviderRegistry.addProvider(RawLayoutProvider.ALIAS, this);
        log.info("Registered sync artifact '{}' with alias '{}'.",
                getClass().getCanonicalName(), RawLayoutProvider.ALIAS);
    }
    ...
    public void fullSync(SyncArtifactForm syncArtifactForm) {
        try {
            ...
            log.info("[{}] repositoryBaseUri [{}] raw存放爬取信息的目录 [{}]", this.getClass().getSimpleName(), repositoryBaseUri, dirPath);
            File dir = new File(dirPath);
            if (!dir.exists()) {
                boolean flag = dir.mkdirs();
                log.info("[{}] raw存放爬取信息的目录 [{}] 不存在，创建状态 [{}]", this.getClass().getSimpleName(), dirPath, flag);
            }
            ...
            if (!rootFile.exists()) {
                ...
                log.info("[{}] raw存放爬取信息的文件 [{}] 不存在，创建状态 [{}]", this.getClass().getSimpleName(), rootFile.getAbsolutePath(), flag);
            ...
            while ((urlFile = getLevelFile(dir, level)).exists()) {
                ...
                try (Scanner scanner = new Scanner(urlFile);
                    ...
                    log.error("[{}] raw全量同步制品错误 [{}]", this.getClass().getSimpleName(), ExceptionUtils.getStackTrace(e));
                }
                if (fileEmpty) {
            ...
            log.info("[{}] raw全量同步制品耗时 [{}] ms, 同步制品总个数 [{}]", this.getClass().getSimpleName(), System.currentTimeMillis() - startTime, THREAD_LOCAL.get());
        } catch (Exception e) {
            log.error("[{}] raw全量同步制品，fullSync错误 [{}]", this.getClass().getSimpleName(), ExceptionUtils.getStackTrace(e));
        } finally {
            THREAD_LOCAL.remove();
        }
    }
    ...
    private void findSubUrl(Repository repository, String rootUrl, String url, String remoteUrl, Integer sleepMillis, String dom, FileWriter writer, String repositoryBaseUri) {
        try {
            log.info("[{}] raw全量同步 findSubUrl url [{}]", this.getClass().getSimpleName(), url);
            ...
            log.info("[{}] raw全量同步 findSubUrl links [{}]", this.getClass().getSimpleName(), links.toString());
            for (Element link : links) {
                String absUrl = link.absUrl("href");
                if (rawLayoutProvider.isChecksum(absUrl)) {
                    log.info("[{}] raw全量同步 findSubUrl absUrl [{}] is checksum", this.getClass().getSimpleName(), absUrl);
                    continue;
                }
                if (!absUrl.endsWith(separator)) {
                    absUrl = absUrl.replace(remoteUrl, repositoryBaseUri);
                    log.info("[{}] raw全量同步 findSubUrl absUrl [{}]", this.getClass().getSimpleName(), absUrl);
                    HttpUtil.get(absUrl);
            ...
            log.error("[{}] raw全量同步制品，findSubUrl错误 [{}]", this.getClass().getSimpleName(), ExceptionUtils.getStackTrace(e));
        }
    }
}
