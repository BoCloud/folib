...
public class PathSpecificBasicAuthenticationEntryPoint extends BasicAuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException {
        ...
        if (path.startsWith("/dav/")) {
            ...
            handler401(request, response, authException);
        }
    }
    ...
    public static void handler401(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException {
        String message = Optional.ofNullable(authException).map(Throwable::getMessage).orElse("unauthorized");
        ...
        response.getWriter().println(SpringUtil.getBean(ObjectMapper.class).writeValueAsString(new ErrorResponseEntityBody(message)));
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        response.flushBuffer();
    }

    private static void setDockerTokenUrl(HttpServletRequest request, HttpServletResponse response, String userAgent) {
