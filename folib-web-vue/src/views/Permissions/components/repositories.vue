<template>
    <a-modal
        v-model="visible"
        :title="$t(`Permissions.${isEdit ? 'EditRepositories' : 'AddRepositories'}`)"
        width="50vw"
        @close="closeModal"
    >
        ...
        <a-steps v-model="step" type="navigation" size="small" class="step">
            <a-step
                :title="$t('Permissions.SelectRepositories')"
                :status="step === 0 ? 'process' : 'wait'"
                :description="$t('Permissions.SelectRepositoriesDesc')"
            />
            <a-step
        ...
        <div v-else>
            <a-radio-group v-model="radioModel">
                <a-radio-button value="ALL">
                    {{ $t('Permissions.AllRepositories') }}
                </a-radio-button>
                <a-radio-button value="PER">
                    {{ $t('Permissions.PerRepository') }}
                </a-radio-button>
            </a-radio-group>
...
export default {
    ...
    data()
    {
        return {
            ...
            currentExPattern: '',
            excludePatterns: [],
            exForm: this.$form.createForm(this, { name: 'repositoriesEx' }),
            searchText: '',
            searchFocus: false,
