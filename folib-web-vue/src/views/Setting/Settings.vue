        ...
        <a-row type="flex" :gutter="24">
          ...
          <a-col :span="24" :md="12" class="mb-24">
            <a-card :bordered="false" class="header-solid h-full card-profile-information" :bodyStyle="{paddingTop: 0, paddingBottom: '16px' }" :headStyle="{paddingRight: 0,}">
              ...
              <a-button type="link" slot="extra" @click="postActivate">
              ...
              <p class="$color-muted">
                你可以复制左侧机器码来获取FOLIB的序列号，并将序列号填入下方。请确保激活过程可以联网，如果需要开通网络策略，请将"license.folib.com"设置为白名单
              </p>
              <hr class="my-25">
  ...
  methods: {
    ...
    postActivate(){
      if(this.activateCode){
        postActivate(this.activateCode).then(res=>{
          if(res.rel){
            setTimeout(() => {
              this.$notification.success({
                message: '复制成功'
