export default {
    ...
    'CondaGlobalConfiguration': '要将Conda客户端指向FoLib仓库，请编辑用户目录下的配置文件（例如~/.condarc）：',
    'CondaDeploy': '发布',
    'CondaDeployConfig': '要发布一个包并将其上传到FoLib，请使用curl命令。',
    'CondaInstall': '安装',
    'CondaInstallConfig': '要安装一个包，请使用conda install命令。',
    'OllamaInstallConfig': '要安装一个 model，请使用 ollama run 命令。',
    'CjpmInstallCommand': '安装cjpmp命令',
    'CjpmInstallCommandDetail': 'cjpm 暂不支持从Folib导入依赖，需要下载额外的命令工具。',
    'CjpmConfiguration': '全局配置',
    'CjpmConfigurationDetail': '通过环境变量设置Folib地址以及访问令牌',
    'CjpmPublish': '发布Cangjie包',
    'CjpmPublishDetail': '使用cjpmp publish 发布一个包并上传至Folib',
    'CjpmImport': '引入依赖',
    'CjpmImportDetail': '要使用制品库中的Cangjie包，需要在cjpm.toml文件中进行如下配置，并使用cjpmp build命令编译项目',
    'Recursive': '递归',
