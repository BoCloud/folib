<template>
	...
	<a-card :bordered="false" class="dashboard-bar-chart">
		...
		<div class="card-title">
			<h6>filesSize</h6>
			<p>than last week 
				<span v-if="latestData.previousDiff > 0" class="text-success">+ {{ fileSizeConver(latestData.previousDiff) }}</span>
				<span v-else-if="latestData.previousDiff === 0"></span>
				<span v-else class="text-danger">- {{ fileSizeConver(latestData.previousDiff) }}</span>
			</p>
		</div>
